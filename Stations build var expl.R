...
#pour avoir les altitudes et la cl? d'API associ?e
library(googleway)
...
#r?cup?ration de la liste officielle des stations
#################################################
...
# et cr?ation comme une v?ritable variable
...
#r?cup?ration de la hauteur des stations
#########################################
...
i<-0
#on fait une boucle car le programme e g?re pas toue la liste des stations d'un coup
...
# on compl?te par les statins restantes
...
#sortie : une liste de vecteurs de num?ro de stations (num?ros dans l'ordre de la matrice station)
# (pour la station i, listedist[[i]] est un vecteur des stations proches de la station de la i?me ligne de la matrice stations)
#la matrice station est consid?r?e comme une variable globale
...
#argument : nombre n de stations ? retenir
#sortie : une matrice de nrow(stations) lignes et n colonnes
# la premi?re colonne donne la station la plus proche parmi les n et la n est la plus ?loign?e
#la matrice station est consid?r?e comme une variable globale
...
# (+ ou - notion de densit? de bike_stand)
...
# Distance ? la station de m?tro la plus proche
#################################################

#r?cup?ration des donn?es RATP stock?es dans le fichier coordRATP.json
...
#cr?ation de la variable $RATP
#distance ? la sation de m?tro/RER/tram la plus proche
for (i  in (1:nrow(stations)))
{
...
# R?cup?ration de la base de donn?e ?quipement
#################################################
...
# r?sultat : il n'y a pas tout. En services et commerces, ce n'est pas trop grave car 
# ce ne sont pas les variables a priori les + pertinentes (? v?rifier ex post)
# sinon quelques ?quipements autres non g?olocalis?s mais en faible proportion
# sauf les cin?mas qui sont donc la variable a priori la plus manquante
# les bases des ann?es pr?c?dentes n'apportent pas plus d'info

#enrichissement de la base par des cat?gories plus grosses choisies ? la main
equip$typequ2<-""
...
equip[substring(equip$typequ,1,2)== "C1","typequ2"]<-"C1" # ?coles maternelles et ?l?mentaires
equip[substring(equip$typequ,1,2)== "C2","typequ2"]<-"C2" # coll?ge
equip[substring(equip$typequ,1,2)== "C3","typequ2"]<-"C3" # Lyc?es
equip[substring(equip$typequ,1,2)== "C4","typequ2"]<-"C4" # Formations post bac non universitaire
equip[substring(equip$typequ,1,2)== "C5","typequ2"]<-"C5" # Enseignement sup?rieur
equip[substring(equip$typequ,1,2)== "C6","typequ2"]<-"C6" # Autres formations
equip[substring(equip$typequ,1,2)== "C7","typequ2"]<-"C7" # r?sidence et restau U
equip[substring(equip$typequ,1,1)== "D","typequ2"]<-"D" # Soin et social
equip[substring(equip$typequ,1,2)== "D1","typequ2"]<-"D1" # Hopitaux et assimil?s
equip[substring(equip$typequ,1,2)== "E1","typequ2"]<-"E1" # Taxis
equip[substring(equip$typequ,1,2)== "E2","typequ2"]<-"E2" # A?roports
...
equip[substring(equip$typequ,1,2)== "F3","typequ2"]<-"F3" # Culture (Cin? th?atre mus?es)
...
#fonction qui convertit les coordonn?es lambert 93  en latitude longitude
...
#fonction qui donne la distance minimale entre une coordonn?es et un ?quipement de type "type"
#@param coord : un vecteur de lat/lon,
#type un type d'?quipement correspondant ? la colonne typequ2 de la matrice equip (ex "E2")
