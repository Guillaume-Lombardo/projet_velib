...
### clustering kmeans #############################################################################
## ACP sur les series temps ####
station_PCA <- PCA(X = station_x_date_mod7j[ ,2:ncol(station_x_date_mod7j)],ncp = ncol(station_x_date_mod7j)-1)

# plot.PCA(x = station_PCA,axes = c(1,2),choix = 'var',label = 'none')
# plot.PCA(x = station_PCA,axes = c(1,3),choix = 'var',label = 'none')
# plot.PCA(x = station_PCA,axes = c(2,3),choix = 'var',label = 'none')
# plot.PCA(x = station_PCA,axes = c(1,4),choix = 'var',label = 'none')
# plot.PCA(x = station_PCA,axes = c(1,2),label = 'none',label = 'none')
# plot.PCA(x = station_PCA,axes = c(1,3),label = 'none',label = 'none')
# plot.PCA(x = station_PCA,axes = c(2,3),label = 'none',label = 'none')
# plot.PCA(x = station_PCA,axes = c(1,2),label = 'none',label = 'none')
df_variance <- data.frame(nombre_axe = seq_along(station_PCA$eig$`cumulative percentage of variance`),part_variance = station_PCA$eig$`cumulative percentage of variance`)
head(which(station_PCA$eig$`cumulative percentage of variance`>=94),1)
...
scaled_series <- base::scale(as.matrix(station_x_date_mod7j[ ,2:ncol(station_x_date_mod7j)]))
nouvelle_series <- scaled_series %*% t(Ancien_vers_nouveau) #* attr(scaled_series,'scaled:scale') + attr(scaled_series,'scaled:center')
...
difference <- lapply(1:10, function(.x) cluster[[.x]]$size)
sinkhole <- lapply(1:10,function(.x) print(c(.x,difference[[.x]])))
sinkhole <- lapply(1:10,function(.x) print(c(.x,round(1199/.x),min(difference[[.x]]),max(difference[[.x]]))))


cluster2 <- lapply(1:10,function(.x) kmeans(nouvelle_series[,1:40],.x,iter.max = 50))
representation_kmeans2 <- lapply(1:10,function(.x) merge(data.frame(number = station_x_date_mod7j$number,
																																	 ...
																																	 station_x_date_mod7j[,-1]), 
																												stations_gps, by="number", all.x=F))
difference <- lapply(1:10, function(.x) cluster2[[.x]]$size)
sinkhole <- lapply(1:10,function(.x) print(c(.x,difference[[.x]])))
sinkhole <- lapply(1:10,function(.x) print(c(.x,round(1199/.x),min(difference[[.x]]),max(difference[[.x]]))))
...
moyenne_par_classe <- function(representation_kmeans,x){
...
description_classe <- lapply(1:10,function(.x) moyenne_par_classe(representation_kmeans,.x))
...
description_classe2 <- lapply(1:10,function(.x) moyenne_par_classe(representation_kmeans2,.x))
profil2_par_classe <- extraction_liste(description_classe2,1)
profil2_p05_classe <- extraction_liste(description_classe2,2)
profil2_p10_classe <- extraction_liste(description_classe2,3)
profil2_p25_classe <- extraction_liste(description_classe2,4)
profil2_p50_classe <- extraction_liste(description_classe2,5)
profil2_p75_classe <- extraction_liste(description_classe2,6)
profil2_p90_classe <- extraction_liste(description_classe2,7)
profil2_p95_classe <- extraction_liste(description_classe2,8)
...
for(i in 2:10){
	write.table(x = representation_kmeans2[[i]][,c('number','cluster')],
							file = paste('./Sortie/clustering_ACP_',i,'_classes_mod7j.csv',sep = ''),
							sep = ';',dec = ',',row.names = F)
}
...
eval(parse(text = paste0('profil_colonnes_',1:10,' <- data.frame(t(profil_par_classe[[',1:10,']][,2:505]))')))
eval(parse(text = paste0('names(profil_colonnes_',1:10,') <- paste("P",seq_along(names(profil_colonnes_',1:10,')),sep ="")')))
eval(parse(text = paste0('profil_colonnes_',1:10,'$time <- pre_dates')))
eval(parse(text = paste0('rownames(profil_colonnes_',1:10,') <- NULL')))
...
##
eval(parse(text = paste0('profil_2colonnes_',1:10,' <- data.frame(t(profil2_par_classe[[',1:10,']][,2:505]))')))
eval(parse(text = paste0('names(profil_2colonnes_',1:10,') <- paste("P",seq_along(names(profil_2colonnes_',1:10,')),sep ="")')))
eval(parse(text = paste0('profil_2colonnes_',1:10,'$time <- pre_dates')))
eval(parse(text = paste0('rownames(profil_2colonnes_',1:10,') <- NULL')))

eval(parse(text = paste0('p05_2colonnes_',1:10,' <- data.frame(t(profil2_p05_classe[[',1:10,']][,2:505]))')))
eval(parse(text = paste0('names(p05_2colonnes_',1:10,') <- paste0("P",seq_along(names(p05_2colonnes_',1:10,')),"_p05")')))
eval(parse(text = paste0('p05_2colonnes_',1:10,'$time <- pre_dates')))
eval(parse(text = paste0('rownames(p05_2colonnes_',1:10,') <- NULL')))

eval(parse(text = paste0('p10_2colonnes_',1:10,' <- data.frame(t(profil2_p10_classe[[',1:10,']][,2:505]))')))
eval(parse(text = paste0('names(p10_2colonnes_',1:10,') <- paste0("P",seq_along(names(p10_2colonnes_',1:10,')),"_p10")')))
eval(parse(text = paste0('p10_2colonnes_',1:10,'$time <- pre_dates')))
eval(parse(text = paste0('rownames(p10_2colonnes_',1:10,') <- NULL')))
...
eval(parse(text = paste0('names(p50_2colonnes_',1:10,') <- paste0("P",seq_along(names(p50_2colonnes_',1:10,')),"_p50")')))
eval(parse(text = paste0('p50_2colonnes_',1:10,'$time <- pre_dates')))
eval(parse(text = paste0('rownames(p50_2colonnes_',1:10,') <- NULL')))

eval(parse(text = paste0('p75_2colonnes_',1:10,' <- data.frame(t(profil2_p75_classe[[',1:10,']][,2:505]))')))
eval(parse(text = paste0('names(p75_2colonnes_',1:10,') <- paste0("P",seq_along(names(p75_2colonnes_',1:10,')),"_p75")')))
eval(parse(text = paste0('p75_2colonnes_',1:10,'$time <- pre_dates')))
eval(parse(text = paste0('rownames(p75_2colonnes_',1:10,') <- NULL')))

eval(parse(text = paste0('p90_2colonnes_',1:10,' <- data.frame(t(profil2_p90_classe[[',1:10,']][,2:505]))')))
eval(parse(text = paste0('names(p90_2colonnes_',1:10,') <- paste0("P",seq_along(names(p90_2colonnes_',1:10,')),"_p90")')))
eval(parse(text = paste0('p90_2colonnes_',1:10,'$time <- pre_dates')))
eval(parse(text = paste0('rownames(p90_2colonnes_',1:10,') <- NULL')))

eval(parse(text = paste0('p95_2colonnes_',1:10,' <- data.frame(t(profil_p95_classe[[',1:10,']][,2:505]))')))
eval(parse(text = paste0('names(p95_2colonnes_',1:10,') <- paste0("P",seq_along(names(p95_2colonnes_',1:10,')),"_p95")')))
eval(parse(text = paste0('p95_2colonnes_',1:10,'$time <- pre_dates')))
eval(parse(text = paste0('rownames(p95_2colonnes_',1:10,') <- NULL')))
##

### export des objets au format RDS pour reutilisation dans l'application shiny ! #################


saveRDS(object = cluster,file = './Sortie/cluster.RDS',compress = 'xz')
saveRDS(object = cluster2,file = './Sortie/cluster2.RDS',compress = 'xz')
saveRDS(object = representation_kmeans,file = './Sortie/representation_kmeans.RDS',compress = 'xz')
saveRDS(object = representation_kmeans2,file = './Sortie/representation_kmeans2.RDS',compress = 'xz')
...
eval(parse(text = paste("saveRDS(object = p95_colonnes_",1:10,",",
												...
												"compress = 'xz')", sep = '')))
eval(parse(text = paste("saveRDS(object = profil_2colonnes_",1:10,",",
												"file = './Sortie/profil_2colonnes_",1:10,".RDS',",
												"compress = 'xz')", sep = '')))
eval(parse(text = paste("saveRDS(object = p05_2colonnes_",1:10,",",
												"file = './Sortie/p05_2colonnes_",1:10,".RDS',",
												"compress = 'xz')", sep = '')))
eval(parse(text = paste("saveRDS(object = p10_2colonnes_",1:10,",",
												"file = './Sortie/p10_2colonnes_",1:10,".RDS',",
												"compress = 'xz')", sep = '')))
eval(parse(text = paste("saveRDS(object = p25_2colonnes_",1:10,",",
												"file = './Sortie/p25_2colonnes_",1:10,".RDS',",
												"compress = 'xz')", sep = '')))
eval(parse(text = paste("saveRDS(object = p50_2colonnes_",1:10,",",
												"file = './Sortie/p50_2colonnes_",1:10,".RDS',",
												"compress = 'xz')", sep = '')))
eval(parse(text = paste("saveRDS(object = p75_2colonnes_",1:10,",",
												"file = './Sortie/p75_2colonnes_",1:10,".RDS',",
												"compress = 'xz')", sep = '')))
eval(parse(text = paste("saveRDS(object = p90_2colonnes_",1:10,",",
												"file = './Sortie/p90_2colonnes_",1:10,".RDS',",
												"compress = 'xz')", sep = '')))
eval(parse(text = paste("saveRDS(object = p95_2colonnes_",1:10,",",
												"file = './Sortie/p95_2colonnes_",1:10,".RDS',",
												"compress = 'xz')", sep = '')))